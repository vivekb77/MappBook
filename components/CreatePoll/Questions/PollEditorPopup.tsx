...
const PollEditorPopup: React.FC<PollEditorPopupProps> = ({ 
  ...
  const handleSaveAndGenerate = () => {
    // Note: This will eventually call a Supabase API route
    // The actual implementation for Supabase will be added later
  ...
  const renderPollDetails = () => {
    return (
      <div>
        ...
        <div className="mb-4">
          <label htmlFor="pollName" className="block text-gray-300 mb-2">Poll Name: <span className="text-red-500">*</span></label>
          <input
            type="text"
            id="pollName"
            value={formData.name}
            onChange={(e) => handleInputChange('name', e.target.value)}
            className={`w-full bg-gray-700 border ${errors.name ? 'border-red-500' : 'border-gray-600'} rounded-lg px-4 py-2 text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500`}
      ...
      <div>
        ...
        <div className="bg-gray-700 p-4 rounded-lg mb-6">
          <div className="mb-4">
            ...
            <p className="text-gray-300"><span className="font-medium">Name:</span> {formData.name}</p>
